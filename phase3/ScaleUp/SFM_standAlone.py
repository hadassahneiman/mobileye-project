...
class FrameContainer(object):
    def __init__(self, img_path=None):
        ...
        self.img = plt.imread(img_path) if img_path else None
        self.traffic_light = []
...
print(curr_container.traffic_light[0])
EM = np.eye(4)
...
def calc_distances(image_path, curr_candidates, prev_candidates, data_holder):
    ...
    curr_container.EM = data_holder.m_EM
    curr_container = SFM.calc_TFL_dist(prev_container, curr_container, data_holder.focal, data_holder.principle_point)
    visualize(prev_container, curr_container, data_holder.focal, data_holder.principle_point)
